...
for (i in 1:i.iterations){  
  ...
  # estimate model
  model.fit   <- train(PRI_DE ~ (CON_DE + PRO_DE_WND + PRO_DE_SPV)*Peak*Weekday*Summer + CON_FR + factor(Month) + factor(Hour) + factor(Weekday) + Lag_D_PRI_DE,
                       data = d.est,
...
write.csv(as.data.frame(container.forecast), file='../forecasts/test/svm-radial.csv')
